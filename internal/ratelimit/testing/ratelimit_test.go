...
func TestTokenBucket_BasicFunctionality(t *testing.T) {
	...
	tb := ratelimit.NewTokenBucket(storage, 10, 5, time.Minute) // 10 capacity, 5 tokens per minute
...
func TestTokenBucket_Refill(t *testing.T) {
	storage := NewMockStorage()
	tb := ratelimit.NewTokenBucket(storage, 10, 60, time.Minute) // 10 capacity, 60 tokens per minute (1 per second)
	ctx := context.Background()
...
func TestTokenBucket_CapacityLimit(t *testing.T) {
	storage := NewMockStorage()
	tb := ratelimit.NewTokenBucket(storage, 5, 10, time.Minute) // 5 capacity, 10 tokens per minute
	ctx := context.Background()
...
func TestTokenBucket_Reset(t *testing.T) {
	storage := NewMockStorage()
	tb := ratelimit.NewTokenBucket(storage, 10, 5, time.Minute)
	ctx := context.Background()
...
func TestTokenBucket_ConcurrentAccess(t *testing.T) {
	storage := NewMockStorage()
	tb := ratelimit.NewTokenBucket(storage, 100, 10, time.Minute)
	ctx := context.Background()
...
func TestLimiter_BasicFunctionality(t *testing.T) {
	storage := NewMockStorage()
	limiter := ratelimit.NewLimiter(storage, 60, 10) // 60 per minute, 10 burst
	ctx := context.Background()
...
func TestLimiter_RateLimitEnforcement(t *testing.T) {
	storage := NewMockStorage()
	limiter := ratelimit.NewLimiter(storage, 5, 5) // 5 per minute, 5 burst
	ctx := context.Background()
...
func TestLimiter_Reset(t *testing.T) {
	storage := NewMockStorage()
	limiter := ratelimit.NewLimiter(storage, 5, 5)
	ctx := context.Background()
...
func TestLimiter_ConcurrentAccess(t *testing.T) {
	storage := NewMockStorage()
	limiter := ratelimit.NewLimiter(storage, 100, 50)
	ctx := context.Background()
...
func TestLimiter_EdgeCases(t *testing.T) {
	storage := NewMockStorage()
	limiter := ratelimit.NewLimiter(storage, 10, 10)
	ctx := context.Background()
...
func TestTokenBucket_StorageErrors(t *testing.T) {
	...
	tb := ratelimit.NewTokenBucket(errorStorage, 10, 5, time.Minute)
	ctx := context.Background()
...
func BenchmarkTokenBucket_Allow(b *testing.B) {
	storage := NewMockStorage()
	tb := ratelimit.NewTokenBucket(storage, 1000, 100, time.Minute)
	ctx := context.Background()
...
func BenchmarkLimiter_Allow(b *testing.B) {
	storage := NewMockStorage()
	limiter := ratelimit.NewLimiter(storage, 1000, 100)
	ctx := context.Background()
